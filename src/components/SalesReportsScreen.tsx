...
export default function SalesReportsScreen({ sales, onAddSale, onRemoveSale }: SalesReportsScreenProps) {
  ...
  // Add Sale Form state
  const [newItemName, setNewItemName] = useState('Artisanal Sourdough');
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-12">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h2 className="text-4xl font-extrabold text-slate-900 tracking-tight font-display">{t('sales.title')}</h2>
          <p className="text-slate-500 font-medium mt-1">{t('sales.sub')}</p>
        </div>
        <div className="flex gap-3 w-full md:w-auto">
          <div className="relative flex-1 md:flex-none">
            ...
            <select 
              ...
              className="w-full md:w-auto pl-10 pr-10 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 shadow-sm focus:border-emerald-500 outline-none appearance-none cursor-pointer"
            >
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 active:scale-95 transition-all shadow-sm shrink-0 cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden flex flex-col justify-between hover:shadow-md transition-all">
          <div className="flex justify-between items-start mb-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t('sales.dailyAvg')}</h3>
            <span className="p-2 rounded-xl bg-slate-50 text-slate-600"><TrendingUp size={16} /></span>
          </div>
          <div>
            <div className="text-3xl font-black text-slate-900 font-display mb-1">
            ...
            <div className="flex items-center gap-1 text-xs font-bold text-emerald-600">
              <span>+5.2%</span>
              <span className="text-slate-400 font-medium">{t('sales.vsLastWeek')}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden flex flex-col justify-between hover:shadow-md transition-all">
          <div className="flex justify-between items-start mb-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t('sales.avgMargin')}</h3>
            <span className="p-2 rounded-xl bg-slate-50 text-slate-600"><Percent size={16} /></span>
          </div>
          <div>
            <div className="text-3xl font-black text-slate-900 font-display mb-1">{avgMargin.toFixed(1)}%</div>
        ...
        <div className="bg-slate-900 p-6 rounded-3xl text-white border border-slate-800 shadow-xl hover:scale-[1.01] transition-all flex flex-col justify-between cursor-pointer group">
          <div className="flex justify-between items-start mb-4">
            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">{t('sales.topItem')}</h3>
            <span className="p-2 rounded-xl bg-slate-800 text-slate-400"><Star size={16} /></span>
          </div>
          <div>
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
        ...
        <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white">
          <div className="flex items-center gap-3">
            <h3 className="text-lg font-bold text-slate-900 font-display">{t('sales.itemizedReport')}</h3>
            <button 
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input 
              ...
              type="text"
              className="w-full pl-9 pr-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 focus:border-emerald-500 outline-none"
            />
          </div>
        </div>
        ...
        {showAddForm && (
          <form onSubmit={handleAddSubmit} className="p-6 bg-slate-50/80 border-b border-slate-100 grid grid-cols-1 md:grid-cols-5 gap-4 items-end animate-fade-in">
            <div className="md:col-span-1">
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">{t('sales.itemName')}</label>
              <select
                value={newItemName}
                onChange={(e) => setNewItemName(e.target.value)}
                className="w-full bg-white border border-slate-200 rounded-xl p-2.5 text-sm text-slate-800 outline-none focus:border-emerald-500"
              >
                ...
                <option value="House Salad">{translateItem('House Salad')}</option>
                <option value="Seared Scallops">{translateItem('Seared Scallops')}</option>
                <option value="Wagyu Burger">{translateItem('Wagyu Burger')}</option>
              </select>
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">{t('sales.quantity')}</label>
              <input 
                type="number"
                min="1"
                value={newQty}
                onChange={(e) => {
                  ...
                  setNewQty(q);
                  // Approximate pricing
                  if (newItemName === 'Artisanal Sourdough') { setNewRevenue(q * 9.00); setNewCost(q * 2.63); }
                  else if (newItemName === 'Braised Ribs') { setNewRevenue(q * 45.00); setNewCost(q * 14.40); }
                  else if (newItemName === 'Sous Vide Salmon') { setNewRevenue(q * 35.00); setNewCost(q * 12.85); }
                  else if (newItemName === 'Truffle Risotto') { setNewRevenue(q * 28.00); setNewCost(q * 7.00); }
                  else { setNewRevenue(q * 12.00); setNewCost(q * 2.00); }
                }}
                className="w-full bg-white border border-slate-200 rounded-xl p-2.5 text-sm text-slate-800 outline-none focus:border-emerald-500"
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">{t('sales.revenue')}</label>
              <input 
                type="number"
                step="0.01"
                value={newRevenue}
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">{t('sales.totalCost')}</label>
              <input 
                type="number"
                step="0.01"
                value={newCost}
            ...
            <div className="flex gap-2">
              ...
              <button 
                ...
                className="bg-white border border-slate-200 text-slate-600 py-2.5 px-3.5 rounded-xl text-xs font-bold hover:bg-slate-50 transition-all cursor-pointer"
              >
                {t('sales.cancel')}
              </button>
            </div>
        ...
        <div className="overflow-x-auto flex-1">
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 sticky top-0 border-b border-slate-100">
              <tr>
                <th className="py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider">{t('sales.thDate')}</th>
                <th className="py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider">{t('sales.thItem')}</th>
                <th className="py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">{t('sales.thQty')}</th>
                <th className="py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">{t('sales.thRevenue')}</th>
                <th className="py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">{t('sales.thCost')}</th>
                <th className="py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">{t('sales.thMargin')}</th>
                <th className="py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">{t('sales.thActions')}</th>
              </tr>
            </thead>
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/50 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <button 
              ...
              className="p-1.5 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-40 transition-all text-slate-600 cursor-pointer"
            >
              <ChevronLeft size={16} />
            </button>
            <button 
